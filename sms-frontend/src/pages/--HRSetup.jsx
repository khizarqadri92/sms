          ...
          <>
            {canManage&&(
              <div style={{display:"flex",gap:6,marginBottom:12}}>
                ...
                <button className="btn btn-primary btn-sm" style={{color:"#fff",whiteSpace:"nowrap"}}
                  onClick={async()=>{
                    if(!desigForm.name.trim()){showFlash("error","Name required.");return;}
                    try{ await hrApi.createDesignation({name:desigForm.name,department_id:desigForm.department_id||null}); setDesigForm({name:"",department_id:""}); loadAll(); showFlash("success","Designation added."); }
