...
export default function Discipline() {
  ...
  return (
    <div>
      {selectedCaseId && <DisciplineDetailModal caseId={selectedCaseId} onClose={()=>setSelectedCaseId(null)} onActed={()=>setSelectedCaseId(null)} />}
                ...
                {filteredList.map((c,i)=>{
                  ...
                  return (
                    <tr key={c.id} style={{borderBottom:"1px solid #e2e8f0",background:active?"#eff6ff":"#fff",cursor:"pointer",transition:"background .1s"}} onClick={()=>loadDetail(c.id)}>
                      ...
                      <td style={{padding:"12px 14px"}}><button className="btn btn-ghost btn-sm" onClick={e=>{e.stopPropagation();setSelectedCaseId(c.id);}}>View</button></td>
                    </tr>
                  );
                })}
              </tbody>
