...
// Same visual design as DatePicker (themed header, month/year navigation,
// portal-based positioning so it is never clipped by a modal's overflow),
// with a time-of-day row added below the calendar grid. Value/onChange use
// the same "YYYY-MM-DDTHH:MM" shape as a native <input type="datetime-local">,
// so it drops in as a replacement without changing any surrounding state.
export default function DateTimePicker({ value, onChange, disabled, min, max, style, placeholder }) {
  ...
  const [viewDate, setViewDate] = useState(() => (datePart ? new Date(datePart) : new Date()));
  const [coords, setCoords] = useState({ top: 0, left: 0 });
  useEffect(() => { if (!datePart) setViewDate(new Date(processingToday)); }, [processingToday]);
  ...
  const commit = (newDatePart, newTimePart) => {
    const t = newTimePart || timePart || "00:00";
    onChange(newDatePart + "T" + t);
  };
  ...
  const handleTimeChange = (e) => {
    ...
    if (datePart) commit(datePart, t);
    else commit(processingToday, t);
  };

  const isDisabledDay = (d) => {
    if (!d) return true;
    const dateObj = new Date(year, month, d);
    if (min && dateObj < new Date(min.split("T")[0])) return true;
    if (max && dateObj > new Date(max.split("T")[0])) return true;
    return false;
  };

  const weekdayLabels = [...Array(7)].map((_, i) => WEEKDAY_SHORT[(firstDayIndex + i) % 7]);

  const selectedDateObj = datePart ? new Date(datePart) : null;

  const minYear = min ? new Date(min.split("T")[0]).getFullYear() : year - 100;
  const maxYear = max ? new Date(max.split("T")[0]).getFullYear() : year + 20;
  const yearLo = Math.min(minYear, year);
  const yearHi = Math.max(maxYear, year);
  const yearOptions = [];
  for (let y = yearHi; y >= yearLo; y--) yearOptions.push(y);

  const displayValue = datePart ? formatDate(datePart) + (timePart ? " " + timePart : "") : "";
    ...
    <div ref={popupRef} style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 20, fontWeight: 700, marginTop: 4, lineHeight: 1.2 }}>
          {formatDate(selectedDateObj || processingToday)}{timePart ? " " + timePart : ""}
        </div>
      </div>
