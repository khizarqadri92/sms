...
def list_departments(user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_departments()")
    return ok(data=[dict(r) for r in cur.fetchall()])

@router.get("/departments/{dept_id}/allowed-roles")
def get_allowed_roles(dept_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT parent_id FROM departments WHERE id=%s", (dept_id,))
    row = cur.fetchone()
...
def update_department(dept_id: int, body: DepartmentIn, user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
...
def delete_department(dept_id: int, user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
...
def list_designations(user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_designations()")
    return ok(data=[dict(r) for r in cur.fetchall()])

@router.post("/designations")
def create_designation(body: DesignationIn, user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_create_designation(%s,%s)", (body.name, body.department_id))
    result = cur.fetchone()
    if result["error_msg"]:
...
def update_designation(des_id: int, body: DesignationIn, user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
...
def delete_designation(des_id: int, user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
...
def list_staff(
    ...
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_staff(%s,%s,%s,%s)",
        (department_id, status, employment_type, search))
    return ok(data=[fmt(r) for r in cur.fetchall()])

@router.get("/staff/{staff_id}")
def get_staff(staff_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
...
@router.post("/staff")
def create_staff(body: StaffIn, user_id: int = Depends(require_permission("hr.create")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_create_staff(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (body.first_name, body.last_name, body.gender, body.date_of_birth or None,
         ...
         body.salary, body.employee_code, body.user_id, user_id))
    result = cur.fetchone()
...
def update_staff(staff_id: int, body: StaffIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def update_staff_status(staff_id: int, status: str, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def delete_staff(staff_id: int, user_id: int = Depends(require_permission("hr.delete")), db=Depends(get_db)):
    cur = get_cur(db)
...
def upload_document(staff_id: int,
    ...
    user_id: int = Depends(require_permission("hr.documents")), db=Depends(get_db)):
...
def generate_employee_code(role_id: int, user_id: int = Depends(require_permission("hr.create")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT sp_generate_employee_code(%s) AS code", (role_id,))
    row = cur.fetchone()
...
def get_roles_by_dept(dept_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_roles_by_department(%s)", (dept_id,))
    return ok(data=[dict(r) for r in cur.fetchall()])

@router.get("/departments/{dept_id}/designations")
def get_designations_by_dept(dept_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_all_staff(
    ...
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_all_staff(%s,%s,%s)", (department_id, role_id, search))
    return ok(data=[fmt(r) for r in cur.fetchall()])
...
def create_staff_full(body: CreateStaffIn, user_id: int = Depends(require_permission("hr.create")), db=Depends(get_db)):
    import bcrypt
    pwd_hash = bcrypt.hashpw(body.password.encode(), bcrypt.gensalt(12)).decode()
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_create_staff_with_user(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (body.first_name, body.last_name, body.email, body.role_id,
         ...
         body.gender, body.date_of_birth or None))
    result = cur.fetchone()
    if result["error_msg"]: fail(result["error_msg"], 400)
    db.commit()
...
def get_dept_roles(user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("""SELECT dr.id, d.id AS department_id, d.name AS department_name,
        ...
        JOIN roles r ON r.id=dr.role_id ORDER BY d.name, r.name""")
    return ok(data=[dict(r) for r in cur.fetchall()])

@router.post("/setup/department-roles")
def add_dept_role(department_id: int, role_id: int,
    user_id: int = Depends(require_permission("hr.designations")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_profile_by_user(p_user_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
...
def add_staff_education(staff_id: int, body: EducationIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def update_staff_education(staff_id: int, edu_id: int, body: EducationIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def delete_staff_education(staff_id: int, edu_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def add_staff_experience(staff_id: int, body: ExperienceIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def update_staff_experience(staff_id: int, exp_id: int, body: ExperienceIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def delete_staff_experience(staff_id: int, exp_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_profile(staff_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_staff_leave_types(user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
...
def delete_leave_type(lt_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_leave_policies(leave_type_id: Optional[int] = None, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_staff_leave_policies(%s)", (leave_type_id,))
    return ok(data=[dict(r) for r in cur.fetchall()])
...
def delete_leave_policy(policy_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_leave_balances(year: Optional[int] = None, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_staff_leave_balances(%s)", (year,))
    return ok(data=[fmt(r) for r in cur.fetchall()])
...
def get_leave_requests(status: Optional[str] = None, year: Optional[int] = None,
    user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM sp_get_staff_leave_requests(%s,%s)", (status, year))
    return ok(data=[fmt(r) for r in cur.fetchall()])

@router.patch("/leave/requests/{req_id}/review")
def review_leave_request(req_id: int, body: LeaveReviewIn,
    user_id: int = Depends(require_permission("hr.approve")), db=Depends(get_db)):
...
def list_certificate_types(user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM leave_certificate_types WHERE is_active=true ORDER BY name")
    return ok(data=[dict(r) for r in cur.fetchall()])

@router.post("/leave/certificate-types")
def create_certificate_type(body: CertificateTypeIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("""INSERT INTO leave_certificate_types(name, description, is_active)
        VALUES(%s,%s,%s)
...
def delete_certificate_type(ct_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def get_policy_settings(user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT * FROM hr_policy_settings WHERE id=1")
...
def list_validation_rules(user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
...
def create_validation_rule(body: ValidationRuleIn, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
...
def delete_validation_rule(rule_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
    ...
    for r in cur.fetchall():
        ...
        if rule_type == "certificate_required":
            ...
            cur4.execute("""SELECT s.joining_date, s.resignation_accepted_date, s.is_probationary,
                hps.probation_duration_days, hps.notice_period_duration_days
                FROM staff s CROSS JOIN hr_policy_settings hps
...
def apply_leave(body: LeaveApplyIn, user_id: int = Depends(get_current_user_id), db=Depends(get_db)):
...
def generate_staff_code(staff_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
    # Get user_id for this staff
    cur.execute("SELECT user_id, employee_code FROM staff WHERE id=%s", (staff_id,))
    staff = cur.fetchone()
    if not staff: fail("Staff not found.", 404)
    ...
    try:
        cur.execute("SELECT sp_generate_employee_code(%s) AS code", (role_row["role_id"],))
        row = cur.fetchone()
...
def set_department_head(staff_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
...
def remove_department_head(staff_id: int, user_id: int = Depends(require_permission("hr.edit")), db=Depends(get_db)):
    cur = get_cur(db)
    cur.execute("SELECT user_id, department_id FROM staff WHERE id=%s", (staff_id,))
    staff = cur.fetchone()
    if not staff: fail("Staff not found.", 404)
...
def get_department_head(dept_id: int, user_id: int = Depends(require_permission("hr.view")), db=Depends(get_db)):
    cur = get_cur(db)
